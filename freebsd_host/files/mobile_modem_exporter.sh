...
for modem in "$@"; do
    # run ATI
    ATI=$(echo "ATI" | /usr/local/pipeserial_venv/bin/pipeserial "$modem" | grep -v "^\^" | grep -v "^$" | tail +2)
    ...
    # run AT+GSN
    serial=$(echo "AT+GSN" | /usr/local/pipeserial_venv/bin/pipeserial "$modem" | grep -v "^\^" | grep -v "^$" | tail +2 | head -1)

    # run AT+CSQ
    CSQ=$(echo "AT+CSQ" | /usr/local/pipeserial_venv/bin/pipeserial "$modem" | grep -v "^\^" | grep -v "^$" | grep "^+CSQ")
