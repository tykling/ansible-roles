...
xzcat ${LATESTLOG} | grep -v "logfile turned over$" | awk -F\" '{ OFS = FS } $76=$64$76' | /usr/local/bin/goaccess -p /usr/local/etc/goaccess.conf --invalid-requests=/var/log/goaccess-fail-$(basename ${LATESTLOG}).log --restore --db-path=/var/db/goaccess --persist -o /usr/local/www/goaccess/${YEAR}.html
