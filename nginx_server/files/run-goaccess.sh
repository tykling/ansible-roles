...
xzcat ${LATESTLOG} | grep -v "logfile turned over$" | awk -F\" '{ OFS = FS } $76=$64$76' | /usr/local/bin/goaccess -p /usr/local/etc/goaccess.conf --invalid-requests=/var/log/goaccess-fail-$(basename ${LATESTLOG}).log --load-from-disk --db-path=/var/db/goaccess --keep-db-files -o /usr/local/www/goaccess/${YEAR}.html
